    ...
    <div className="h-screen w-screen flex flex-col items-center justify-center bg-stone-200">
      ...
      <div className="flex flex-col w-fit">
        <div className="mt-16 flex grid grid-cols-3 gap-12">
          <div
            onClick={() => setIsVisibleEdit((prev) => !prev)}
            tabIndex={0}
            onKeyDown={() => { }}
            role="button"
            className={`flex transition ease-in-out delay-200 ${!isVisibleEdit ? 'hover:scale-110' : 'scale-110'} group`}
          >
            <div className={`transition-opacity ease-in duration-300 ${isVisibleEdit ? 'opacity-0' : 'opacity-100'}`}>
              <div>
        ...
        ${!isVisibleEdit ? 'opacity-0' : 'opacity-100'} grid grid-rows-${fileNames.length} gap-4`}
            >
              <p className="text-center text-xl">Please select a PDF</p>
              {fileNames.map((val, index) => (
                <button
                  disabled={!isVisibleEdit}
                  type="button"
          ...
          <div
            onClick={() => setIsVisibleSplit((prev) => !prev)}
            tabIndex={0}
            onKeyDown={() => { }}
            role="button"
            className={`flex transition ease-in-out delay-200 ${!isVisibleSplit ? 'hover:scale-110' : 'scale-110'} group`}
          >
            <div className={`transition-opacity ease-in duration-300 ${isVisibleSplit ? 'opacity-0' : 'opacity-100'}`}>
              <div>
                <SplitIcon />
            ...
            <div className={`absolute transition-opacity ease-in duration-300
        ${!isVisibleSplit ? 'opacity-0' : 'opacity-100'} grid grid-rows-${fileNames.length} gap-4`}
            >
              <p className="text-center text-xl">Please select a PDF</p>
              {fileNames.map((val, index) => (
                <button
                  disabled={!isVisibleSplit}
                  type="button"
